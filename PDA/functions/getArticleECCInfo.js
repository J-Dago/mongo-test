exports = async function(storeNumber,futureDateTime, userName, barcode) {
  ...
  const response = await context.http.get({
      url: `https://api.sallinggroup.com/v1-test/mobile-platform-services/GetArticleECCInfo?storeNumber=${storeNumber}&printType=No&FutureDateTime=${futureDateTime}&username=${userName}&barcode=${barcode}`,
      headers: {
